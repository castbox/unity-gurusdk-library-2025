namespace Guru.Ads.Max
{
    ...
    public class MaxRewardedLoader
    {
        ...
        {
            var e = MaxAdEventBundleFactory.BuildRadsImp(adUnitId, adInfo, _adPlacement);
        ...
        private void OnAdLoadFailedEvent(string adUnitId, MaxSdkBase.ErrorInfo errorInfo)
        {
            ...
            Debug.Log($"{_tag} --- RADS load Failed:: ErrorCode:{ errorInfo.Code}  Info:{errorInfo.AdLoadFailureInfo}");

            _isAdLoading = false;
            MaxAdHelper.ReloadAdAsync(_retryCount, ReloadAd);
        }
        /// <summary>
        /// 广告显示失败
        /// </summary>
        ...
        private void OnAdDisplayFailedEvent(string adUnitId, MaxSdkBase.ErrorInfo errorInfo, MaxSdkBase.AdInfo adInfo)
        {
            ...
            _eventObserver.OnEventRadsFailed(evt);
            Debug.Log($"{_tag} --- RADS display failed:: ErrorCode:{ errorInfo.Code}  Info:{errorInfo.AdLoadFailureInfo}");
        ...
        private void OnAdClickedEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
        {
            ...
            // 广告点击
            var evt = MaxAdEventBundleFactory.BuildRadsClick(adUnitId, adInfo, _adPlacement);
        ...
        {
            // 广告收益
            var evt = MaxAdEventBundleFactory.BuildRadsPaid(adUnitId, adInfo, _adPlacement);
        ...
        {
            // 广告成功获得奖励
            var evt = MaxAdEventBundleFactory.BuildRadsRewarded(adUnitId, adInfo, _adPlacement);
