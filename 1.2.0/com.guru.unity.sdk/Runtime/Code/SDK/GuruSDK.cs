...
namespace Guru
{
    ...
    public partial class GuruSDK: MonoBehaviour
    {
        ...
        {
            // Adjust 延迟时间
            if (RemoteConfigManager.Instance.TryGetValue(AdjustService.REMOTE_DELAY_TIME_KEY, out float value, out ValueSource source))
            {
                var dataSource = DelayMinutesSource.Default;
                switch (source)
                {
                    case ValueSource.RemoteValue: 
                        dataSource = DelayMinutesSource.RemoteConfig; 
                        break;
                    case ValueSource.StaticValue: 
                        dataSource = DelayMinutesSource.Local; 
                        break;
                    default: 
                        dataSource = DelayMinutesSource.Default; 
                        break;
                }
                AdjustService.Instance.SetAdRevDelayMinutes(value, dataSource);
            }
