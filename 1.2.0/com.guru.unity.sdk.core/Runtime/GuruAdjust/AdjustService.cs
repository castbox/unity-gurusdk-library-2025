...
namespace Guru
{
	...
	public class AdjustService
	{
		...
		private const float DEFAULT_DELAY_MINUTES = 30;
		...
		public string IDFA
		{
			get
			{
				if(string.IsNullOrEmpty(_idfa)) _idfa = Adjust.getIdfa();
				return _idfa;
			}
		}
		...
		public string IDFV
		{
			get
			{
				if(string.IsNullOrEmpty(_idfv)) _idfv = Adjust.getIdfv();
				return _idfv;
			}
		}

		public string SdkVersion => Adjust.getSdkVersion();
...
*/
		...
		*/
		...
		{
			// 使用
			_adSourceName = AdjustConfig.AdjustAdRevenueSourceAppLovinMAX;
			if (adPlatform != "MAX")
			{
				_adSourceName = AdjustConfig.AdjustAdRevenueSourceIronSource;
			}
			...
			adEvent.setAdRevenueNetwork(adSource);
			adEvent.setAdRevenueUnit(adUnitId);
			adEvent.setAdRevenuePlacement(adPlacement);
		...
		private void TrackAccumulatedAdRevenue()
		{
			...
			accumulatedRevenue.setAdImpressionsCount(_revenueModel.impressionCount);
		...
		public void TrackIapEvent(string token, AdjustIapEvent iapEvent)
		{
			...
			adjustEvent.setProductId(iapEvent.productId);
			adjustEvent.setTransactionId(iapEvent.orderId);
			if (!string.IsNullOrEmpty(iapEvent.purchaseToken))
			{
				adjustEvent.setPurchaseToken(iapEvent.purchaseToken);
			}

			if (!string.IsNullOrEmpty(iapEvent.receipt))
			{
				adjustEvent.setReceipt(iapEvent.receipt);
			}	
			// --- BI 属性对齐 ---
