...
namespace Guru.IAP
{
    public class IAPServiceV5: IGuruIapService
    {
        ...
        private void AddStoreCallbacks()
        {
            ...
            _storeController.FetchPurchasesFailed += OnFetchPurchasesFailed;
            _storeController.FetchPurchasesSuccess += OnFetchPurchasesSuccess;
        }
...
// #elif UNITY_ANDROID
        ...
        public bool IsProductHasReceipt(string productName)
        {
            ...
            if (product != null) return  product.hasReceipt ||_storeController.IsExistReceipt(product.definition.id);
            return false;
        }
        ...
        private void OnFetchPurchasesFailed(PurchasesFetchFailureDescription failureDescription)
        {
            OnRestoreHandle(false, failureDescription.message);
        }

        private void OnFetchPurchasesSuccess(Orders orders)
        {
        ...
        {
            Log.I($"--- Restore complete: {success}: msg:{msg}" );
            ...
            if (success)
            {
                ...
                // 扫描所有商品, 追加用户属性
                for (int i = 0; _fetchedProducts != null && i < _fetchedProducts.Count; i++)
                {
                    var product = _fetchedProducts[i];
                    if (product.definition.type != ProductType.Consumable && (product.hasReceipt || _storeController.IsExistReceipt(product.definition.id)))
                    {
                        isIAPUser = true;
                    }
                }
    ...
    internal class GuruStoreController
    {
        ...
        internal event Action<Orders> FetchPurchasesSuccess;
        ...
        private void OnFetchedPurchasesAction(Orders orders)
        {
            if (orders.PendingOrders.Count > 0)
            {
                ...
                {
                    //TODO: 完善逻辑
                    _purchaseService.ConfirmOrder(pendingOrder);
                }
            }
            ...
            if (orders.ConfirmedOrders.Count > 0)
            {
                foreach (var confirmedOrder in orders.ConfirmedOrders)
                {
                    ...
                    if (items == null || items.Count < 1)
                        continue;
                    if (items[0].Product== null || items[0].Product.definition == null)
                        continue;
                    _purchasedReceiptDict[items[0].Product.definition.id] = confirmedOrder.Info.Receipt;
                }
            }
            
            FetchPurchasesSuccess.Invoke(orders);
        }
        ...
        /// <returns></returns>
        public bool IsExistReceipt(string prodictId)
        {
