...
namespace Guru
{
    ...
    public class GuruAppsflyerManagerV1 : MonoBehaviour, IAppsFlyerConversionData
    {
        ...
        private static Action<AppsflyerConversionData?> _onGetConversionData;
        public static event Action<AppsflyerConversionData?> OnGetConversionResult
        {
            add => _onGetConversionData += value;
            remove => _onGetConversionData -= value;
        }

        internal static void InvokeOnGetConversionData(AppsflyerConversionData? data)
        {
            _onGetConversionData?.Invoke(data);
        }
        
        public static void InitAppsFlyerMonoBehaviour()
        {
            // 项目组已经到场景里不需要重复挂载
            if (Instance != null)
            {
                Log.W($"[GuruAppsflyerManagerV1] : InitAppsFlyerMonoBehaviour： GuruAppsflyerManagerV1 has already been mounted to the scene, no need to mount it again!");
                return;
            }
                
            Log.W($"[GuruAppsflyerManagerV1] : InitAppsFlyerMonoBehaviour： GuruAppsflyerManager has not been mounted to the scene, starting to mount!");
            var appsflyer = new GameObject("Appsflyer");
            appsflyer.AddComponent<GuruAppsflyerManagerV1>();
        }
